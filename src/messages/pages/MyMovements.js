...
const TransferWall = () => {
  ...
  return (
    <React.Fragment>
      ...
      {!isLoading && loadedMessages && <MessageList items={loadedMessages.filter((message) => 
              (
                message.TransferMessage
                  .toLowerCase()
                  .includes(`${auth.userName}`.toLowerCase()) || message.TransferMessage.toLowerCase().includes(`${auth.userTeam}`.toLowerCase())
              )
            )} />}
    </React.Fragment>
  );
